...
const Datasource = () => {
  ...
  const togglemtmDropdown = () => {
    setIsmtmDropdownOpen(!ismtmDropdownOpen);
  };
  ...
  return (
    <>
      <Header />
      <Sidebar />
      <div className="datasource-top">
      {/*<div className="DS-buttons">
      <button
          className={`ds-square-button ${activeButton === 'source' ? 'active' : ''}`}
          onClick={() => handleButtonClick('source')}
        >
          Source
        </button>
        <button
          className={`ds-square-button ${activeButton === 'data' ? 'active' : ''}`}
